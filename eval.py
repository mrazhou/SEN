...
def evaluate_transformer(args,encoder_image,encoder_feat,decoder, logger, type=None):
    # Load model
    ...
    with torch.no_grad():
        for i, (image_pairs, caps, caplens, allcaps) in enumerate(
            ...
            start_encoder_image = time.time()
            if type == 'SEN':
                img_cat = torch.cat((imgs_A, imgs_B), 1)
                feature = encoder_image(img_cat)
                encoder_out = encoder_feat(img_cat, feature)
            elif type == None or type == 'RSICCformer':
            ...
            extractor_time.update(time.time() - start_encoder_image)
    ...
    logger.write('eval_time:', eval_time)
    logger.write('avg eval_time:', eval_time / len(references))
    logger.write('fps:', 1/(eval_time/len(references)))
    logger.write('extractor_time:', extractor_time.avg)

    logger.write('len(nochange_references):', len(nochange_references))
    logger.write('len(change_references):', len(change_references))
    # Calculate BLEU1~4, METEOR, ROUGE_L, CIDEr scores
    nochange_metric, change_metric = None, None
    if len(nochange_references)>0:
        nochange_metric = get_eval_score(nochange_references, nochange_hypotheses)
        logger.write('nochange_metric:', nochange_metric)
        logger.write("nochange_acc:", nochange_acc / len(nochange_references))
    if len(change_references)>0:
        change_metric = get_eval_score(change_references, change_hypotheses)
        logger.write('change_metric:', change_metric)
        logger.write("change_acc:", change_acc / len(change_references))
    logger.write(".......................................................")
    metrics = get_eval_score(references, hypotheses)
    for metric in metrics:
        logger.write(metric, metrics[metric])
    logger.flush()
...
def get_all_files(folder_path):
    ...
    for file_name in file_list:
        ...
        if os.path.isfile(file_path):
            ...
            yield from get_all_files(file_path)
...
def main(args):
    ...
    for filename in files:
        if '.pth' not in filename:
            continue
        # if (args.epoch in filename) or (args.encoder_feat not in filename)or (args.decoder not in filename) :
        #     continue
        ...
        encoder_feat = checkpoint['encoder_feat'] if 'encoder_feat' in checkpoint else None
        decoder = checkpoint['decoder']
        current_dir = os.path.dirname(filename)
        # with open(os.path.join(current_dir, 'model_arch.txt'), 'a') as f:
        #     print(filename, file=f)
            
        #     get_params = lambda x: sum(p.numel() for p in x.parameters() if p.requires_grad)
        #     print(f'encoder_image params: {get_params(encoder_image):,}', file=f)
        #     print(f"encoder_feat params: {get_params(encoder_feat):,}", file=f) if encoder_feat else None
        #     print(f"decoder params: {get_params(decoder):,}", file=f)
            
        #     print('encoder_image: ', encoder_image, file=f)
        ...
        logger = Logger(os.path.join(current_dir, 'eval.txt'))
        metrics, _1, _2 = evaluate_transformer(args,encoder_image,encoder_feat,decoder,logger, type=args.model)

        logger.write("{} - beam size {}: BLEU-1 {:.4f} BLEU-2 {:.4f} BLEU-3 {:.4f} BLEU-4 {:.4f} METEOR {:.4f} ROUGE_L {:.4f} CIDEr {:.4f}".format
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default="SEN", help='model type')
    parser.add_argument('--encoder_image', default="SEN_Extractor")
    parser.add_argument('--encoder_feat', default="SEN_DiffEncoder")
    parser.add_argument('--decoder', default="SEN_TransDecoder", help="decoder img2txt")  #
    ...
    parser.add_argument('--path', default="models_checkpoint/ablation/proj/512/1", help='model checkpoint.')
