...
class MCCFormers_S(nn.Module):
    ...
    def forward(self, img_feat1, img_feat2):
        ...
        position_embedding = torch.cat([embed_w.unsqueeze(0).repeat(h, 1, 1),
                                       ...
                                       dim=-1)
        #(h, w, d_model)
        position_embedding = position_embedding.permute(2, 0, 1).unsqueeze(
            0).repeat(batch, 1, 1, 1)  # (batch, d_model, h, w)
        ...
        # (batch, d_model, 2*h*w)
        img_feat_cat = torch.cat([img_feat1, img_feat2], dim=2)
        ...
        idx_embedding = idx_embedding.permute(
            1, 0, 2)  # (2*h*w, batch, d_model)
        ...
        img_feat1 = img_feat1.view(
            batch, d_model, -1).permute(2, 0, 1)  # (batch, d_model, h*w)
        img_feat2 = feature[h*w:].permute(1, 2, 0)  # (batch, d_model, h*w)
        img_feat2 = img_feat2.view(
            batch, d_model, -1).permute(2, 0, 1)  # (batch, d_model, h*w)
...
class CrossTransformer(nn.Module):
    ...
    def forward(self, input1, input2):
        ...
        ff_output = self.linear2(self.dropout2(
...
class MCCFormers_D(nn.Module):
    ...
    def __init__(self, feature_dim, dropout, h, w, d_model=512, n_head=4, n_layers=2):
        ...
        self.transformer = nn.ModuleList(
            [CrossTransformer(dropout, d_model, n_head) for i in range(n_layers)])
    ...
    def forward(self, img_feat1, img_feat2):
        ...
        # + position_embedding # (batch_size, d_model, h, w)
        ...
        output1 = img_feat1.view(batch, self.d_model, -1).permute(2, 0, 1)
        # (h*w, batch_size, d_model)
        output2 = img_feat2.view(batch, self.d_model, -1).permute(2, 0, 1)

        for l in self.transformer:
            output1, output2 = l(output1, output2), l(output2, output1)

        position_embedding = position_embedding.view(
            batch, self.d_model, -1).permute(2, 0, 1)
...
class DecoderTransformer(nn.Module):
    ...
    def forward(self, memory, encoded_captions, caption_lengths):
        ...
        mask = (torch.triu(torch.ones(tgt_length, tgt_length))
                == 1).transpose(0, 1)
        ...
        tgt_embedding = self.position_encoding(
            tgt_embedding)  # (length, batch, feature_dim)
...
class PlainDecoder(nn.Module):
    ...
    def __init__(self, feature_dim, embed_dim, vocab_size, hidden_dim, dropout):
        ...
        self.decode_step = nn.LSTMCell(
            embed_dim + feature_dim, hidden_dim, bias=True)
    ...
    def forward(self, l_total, encoded_captions, caption_lengths):
        ...
        for t in range(max(decode_lengths)):
            ...
            c_temp = torch.cat(
                [embeddings[:batch_size_t, t, :], l_total[:batch_size_t]], dim=1)

            h_ds, c_ds = self.decode_step(
                c_temp, (h_ds[:batch_size_t], c_ds[:batch_size_t]))
